...
<div class="flex justify-center">
  <div class="w-full max-w-2xl bg-white rounded-xl shadow-md p-8">
    <div class="mb-6">
      <h2 class="text-2xl font-bold text-blue-700">Edit Sekolah</h2>
      <p class="text-sm text-gray-500">Perbarui data sekolah sesuai kebutuhan.</p>
    </div>

    <form method="POST" action="{{ route('master.schools.update', $school->id) }}">
      @csrf
      @method('PUT')
      ...
      <div class="mb-4">
        ...
        <input type="text" name="type" id="type" value="{{ old('type', $school->type) }}"
               placeholder="misal: formal, diniyah, pondok"
               class="w-full border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring focus:ring-blue-500">
