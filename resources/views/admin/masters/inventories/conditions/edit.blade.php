...
<div class="mb-8">
    <h1 class="text-3xl lg:text-4xl font-extrabold text-gray-900 mb-2 tracking-tight">
        Edit Kondisi Inventaris
    </h1>
    <p class="text-gray-600 text-base">Perbarui detail kondisi inventaris ini.</p>
</div>
...
<div class="bg-white shadow-xl rounded-2xl p-8 mb-8 border border-gray-100">
    <form method="POST" action="{{ route('facility.inventory-conditions.update', $inventoryCondition->id) }}" class="space-y-6">
        @csrf
        @method('PUT')

        <div>
            <label for="name" class="block text-sm font-semibold text-gray-800 mb-1">Nama Kondisi <span class="text-red-500">*</span></label>
            <input type="text" name="name" id="name" value="{{ old('name', $inventoryCondition->name) }}"
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4"
                      class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm py-2.5 px-4 transition-all duration-200"
                      placeholder="Penjelasan lebih detail tentang kondisi ini...">{{ old('description', $inventoryCondition->description) }}</textarea>
        ...
        <div class="flex justify-end items-center gap-4 pt-4">
            ...
            <button type="submit"
                ...
                <i class="fas fa-save mr-2"></i> Perbarui Kondisi
            </button>
        </div>
    </form>
</div>
