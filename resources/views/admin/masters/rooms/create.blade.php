...
<div class="flex justify-center items-start min-h-screen py-10">
  <div class="w-full max-w-xl bg-white p-6 rounded shadow">
    ...
    <h2 class="text-2xl font-bold text-blue-700 mb-1">Tambah Ruangan</h2>
    <p class="text-sm text-gray-500 mb-4">Form untuk menambahkan data ruangan baru.</p>

    <form action="{{ route('facility.rooms.store') }}" method="POST">
      @csrf

      <div class="mb-4">
        <label class="block text-sm font-medium mb-1">name Ruangan</label>
        <input type="text" name="name" value="{{ old('name') }}" class="w-full border rounded px-3 py-2 text-sm" required placeholder="Contoh: Ruang Kelas 7A">
        @error('name')
