...
<div class="mb-8">
    ...
    <p class="text-gray-600 text-base">Isi detail kurikulum baru yang akan ditambahkan ke sistem.</p>
</div>
    ...
    <form action="{{ route('academic.curriculums.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="4"
                      class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm py-2.5 px-4 transition-all duration-200"
                      placeholder="Deskripsi singkat mengenai kurikulum ini.">{{ old('description') }}</textarea>
            @error('description')
                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" id="start_year" name="start_year" value="{{ old('start_year', date('Y')) }}"
                       class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm py-2.5 px-4 transition-all duration-200"
        ...
        <div>
            ...
            <select id="level_group" name="level_group" required
                ...
                {{-- Gunakan data $levelGroups dari controller --}}
                @foreach ($levelGroups as $key => $value)
                    <option value="{{ $key }}" {{ old('level_group') == $key ? 'selected' : '' }}>{{ $value }}</option>
                @endforeach
            </select>
            @error('level_group')
                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
        ...
        {{-- Applicable Grades (assuming single input for comma-separated string, if multi-select needed, will require JS) --}}
        <div>
            ...
            <input type="text" id="applicable_grades" name="applicable_grades" value="{{ is_array(old('applicable_grades')) ? implode(',', old('applicable_grades')) : old('applicable_grades') }}"
                   class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm py-2.5 px-4 transition-all duration-200"
        ...
        <div>
            ...
            <input type="text" id="regulation_number" name="regulation_number" value="{{ old('regulation_number') }}"
                   class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm py-2.5 px-4 transition-all duration-200"
        ...
        <div>
            ...
            <input type="text" id="reference_document" name="reference_document" value="{{ old('reference_document') }}"
                   class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm py-2.5 px-4 transition-all duration-200"
        ...
        <div class="flex justify-end items-center gap-4 pt-4">
            ...
            <button type="submit"
                ...
                <i class="fas fa-save mr-2"></i> Simpan Kurikulum
            </button>
        </div>
    </form>
</div>
