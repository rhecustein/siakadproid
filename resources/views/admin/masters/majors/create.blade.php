...
<div class="mb-8">
    ...
    <p class="text-gray-600 text-base">Isi formulir di bawah ini untuk menambahkan jurusan baru ke dalam sistem.</p>
</div>
...
<div class="bg-white shadow-xl rounded-2xl p-8 mb-8 border border-gray-100">
    <form action="{{ route('academic.majors.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3"
                      class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm text-sm py-2.5 px-4 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
                      placeholder="Deskripsi singkat mengenai jurusan ini.">{{ old('description') }}</textarea>
            @error('description')
                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <select name="type" id="type" required
                ...
                <option value="Akademik" {{ old('type') == 'academic' ? 'selected' : '' }}>Akademik</option>
                <option value="vocational" {{ old('type') == 'vocational' ? 'selected' : '' }}>Kejuruan</option>
