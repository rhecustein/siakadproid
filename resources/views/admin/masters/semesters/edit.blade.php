...
<div class="flex justify-center px-4 py-8">
    <div class="w-full max-w-2xl bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
        ...
        <div class="mb-8 text-center">
            ...
            <p class="text-gray-600 text-base">Perbarui data semester untuk tahun ajaran tertentu.</p>
        </div>

        <form action="{{ route('shared.semesters.update', $semester->id) }}" method="POST" class="w-full">
            @csrf
            @method('PUT')
            ...
            <div class="mb-5">
                ...
                <select name="school_year_id" id="school_year_id"
                    ...
                    @foreach($years as $year)
                        <option value="{{ $year->id }}" {{ old('school_year_id', $semester->school_year_id) == $year->id ? 'selected' : '' }}>
                            {{ $year->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-5">
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $semester->name) }}"
                       ...
                       required placeholder="Contoh: Semester Ganjil, Semester Genap">
                @error('name')
                    <p class="text-sm text-red-600 mt-1.5">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                <label class="inline-flex items-center cursor-pointer">
                    ...
                    <span class="ml-2 text-base text-gray-700 select-none">Tandai sebagai semester aktif</span>
                </label>
                @error('is_active')
                    <p class="text-sm text-red-600 mt-1.5">{{ $message }}</p>
                @enderror
