...
<div class="bg-white shadow-xl rounded-2xl p-8 mb-8 border border-gray-100">
    <form action="{{ route('core.students.store') }}" method="POST" class="space-y-6" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
                <label for="parent_id" class="block text-sm font-semibold text-gray-800 mb-1">Orang Tua / Wali</label>
                <input type="text" id="parent_search" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm py-2.5 px-4 transition-all duration-200" placeholder="Cari nama orang tua...">
        ...
        <div>
            ...
            <textarea name="notes" id="notes" rows="3"
                      class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm py-2.5 px-4 transition-all duration-200"
                      placeholder="Catatan khusus tentang siswa (misal: riwayat kesehatan, dll.)">{{ old('notes') }}</textarea>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let availableParentsCache = []; // Cache for all available parents
        ...
        function toggleUserSection() {
            if (userIdOption.value === 'select_existing') {
                ...
                document.getElementById('user_id').setAttribute('required', 'required'); // Make user_id required
        ...
        async function fetchAvailableUsers() {
            ...
            try {
                const response = await fetch('/api/available-users-for-student', { // API for student users
                    headers: {
        ...
        // --- Parent Search Logic ---
        let availableParentsCache = []; // To store all parents data once fetched

        // Initial fetch of all parents (could be optimized if parents list is very long)
        async function fetchAvailableParents() {
            ...
            try {
                // Adjust this route to your actual API route for searching parents
