...
<div class="mb-8">
    ...
    <p class="text-gray-600 text-base">Perbarui detail peran pengguna ini.</p>
</div>
...
<div class="bg-white shadow-xl rounded-2xl p-8 mb-8 border border-gray-100">
    <form action="{{ route('core.roles.update', $role->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="guard_name" id="guard_name" required
                ...
                <option value="web" {{ old('guard_name', $role->guard_name) == 'web' ? 'selected' : '' }}>Web (Aplikasi Utama)</option>
                <option value="api" {{ old('guard_name', $role->guard_name) == 'api' ? 'selected' : '' }}>API (Aplikasi Mobile/Lainnya)</option>
            </select>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3"
                      class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm py-2.5 px-4 transition-all duration-200"
                      placeholder="Penjelasan singkat tentang tugas dan tanggung jawab peran ini.">{{ old('description', $role->description) }}</textarea>
        ...
        <div>
            ...
            {{-- Mengisi nilai old('scope') dari JSON di database --}}
            <input type="text" name="scope" id="scope"
                   value="{{ old('scope') ? implode(',', old('scope')) : (isset($role->scope) ? implode(',', json_decode($role->scope)) : '') }}"
                   class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm py-2.5 px-4 transition-all duration-200"
