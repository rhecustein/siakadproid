...
<div class="flex justify-center min-h-screen py-10">
  <div class="w-full max-w-2xl bg-white p-6 rounded shadow">
    ...
    <h2 class="text-2xl font-bold text-green-700 mb-1">Edit Jadwal Murojaah</h2>
    <p class="text-sm text-gray-500 mb-4">Perbarui informasi jadwal murojaah siswa.</p>

    <form action="{{ route('religion.quran-review-schedules.update', $schedule->id) }}" method="POST">
      @csrf @method('PUT')
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <div>
          ...
          <input type="time" name="time" value="{{ old('time', $schedule->time) }}" class="w-full border rounded px-3 py-2 text-sm" required>
      ...
      <div class="mb-4">
        ...
        <input type="text" name="target_surah" value="{{ old('target_surah', $schedule->target_surah) }}" class="w-full border rounded px-3 py-2 text-sm" required>
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        <div>
          ...
          <input type="text" name="ayat_start" value="{{ old('ayat_start', $schedule->ayat_start) }}" class="w-full border rounded px-3 py-2 text-sm">
        ...
        <div>
          ...
          <input type="text" name="ayat_end" value="{{ old('ayat_end', $schedule->ayat_end) }}" class="w-full border rounded px-3 py-2 text-sm">
      ...
      <div class="mb-4">
        ...
        <select name="status" class="w-full border rounded px-3 py-2 text-sm" required>
          @foreach(['aktif','selesai','batal'] as $status)
            <option value="{{ $status }}" {{ old('status', $schedule->status) == $status ? 'selected' : '' }}>{{ ucfirst($status) }}</option>
          @endforeach
        </select>
      ...
      <div class="mb-4">
        ...
        <textarea name="note" rows="3" class="w-full border rounded px-3 py-2 text-sm">{{ old('note', $schedule->note) }}</textarea>
