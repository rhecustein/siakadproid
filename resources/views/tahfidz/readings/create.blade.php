...
<div class="flex justify-center min-h-screen py-10">
  <div class="w-full max-w-2xl bg-white p-6 rounded shadow">
    ...
    <h2 class="text-2xl font-bold text-green-700 mb-1">Tambah Kegiatan Mengaji</h2>
    <p class="text-sm text-gray-500 mb-4">Isi data bacaan Al-Qur'an siswa hari ini.</p>

    <form action="{{ route('religion.quran-readings.store') }}" method="POST" enctype="multipart/form-data">
      @csrf
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        <div>
          <label class="block text-sm font-medium mb-1">Metode Mengaji</label>
          <select name="method" class="w-full border rounded px-3 py-2 text-sm" required>
            @foreach(['langsung','dengan guru','daring'] as $method)
      ...
      <div class="mb-4">
        <label class="block text-sm font-medium mb-1">Lampiran Bukti (opsional)</label>
