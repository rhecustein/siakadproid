...
<div class="flex justify-center items-start min-h-screen py-10">
  <div class="w-full max-w-xl bg-white p-6 rounded shadow">
    ...
    <h2 class="text-2xl font-bold text-blue-700 mb-1">Tambah Nilai Ekstrakurikuler</h2>
    <p class="text-sm text-gray-500 mb-4">Form untuk menambahkan nilai ekstrakurikuler siswa.</p>

    <form action="{{ route('academic.grades.extracurricular.store') }}" method="POST">
      @csrf
      ...
      <div class="mb-4">
        ...
        <input type="number" name="score" value="{{ old('score') }}" step="0.01" min="0" max="100"
               class="w-full border rounded px-3 py-2 text-sm" required placeholder="Masukkan nilai">
        @error('score')
          <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
        @enderror
      </div>
      ...
      <div class="mb-4">
        ...
        <textarea name="description" class="w-full border rounded px-3 py-2 text-sm" rows="3"
                  placeholder="Masukkan catatan atau deskripsi...">{{ old('description') }}</textarea>
        @error('description')
          <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
        @enderror
      </div>
