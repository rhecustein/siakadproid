...
<div class="max-w-3xl mx-auto py-6 px-4">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-xl font-bold text-gray-800">Edit Kelas (Grade Level)</h1>
    ...
    <form action="{{ route('academic.grade-levels.update', $gradeLevel->id) }}" method="POST"
        ...
        <div class="grid md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" name="grade" class="w-full border rounded px-3 py-2"
                       value="{{ old('grade', $gradeLevel->grade) }}" min="1" max="20" required>
            </div>
            ...
            <div>
                ...
                <input type="text" name="label" class="w-full border rounded px-3 py-2"
                       value="{{ old('label', $gradeLevel->label) }}" required>
            </div>
        </div>

        <div>
            <label class="block text-sm font-medium mb-1">Deskripsi</label>
            <textarea name="description" rows="3" class="w-full border rounded px-3 py-2">{{ old('description', $gradeLevel->description) }}</textarea>
        </div>

        <div class="flex items-center">
            <input type="checkbox" name="is_active" value="1"
                   class="mr-2" {{ old('is_active', $gradeLevel->is_active) ? 'checked' : '' }}>
