...
<div class="max-w-2xl mx-auto bg-white p-6 rounded shadow">
  <h2 class="text-xl font-bold text-blue-700 mb-4">Input Nilai Siswa</h2>

  <form action="{{ route('academic.grade-details.store') }}" method="POST">
    @csrf
    ...
    <div class="mb-4">
      ...
      <select name="grade_input_id" id="grade_input_id" class="mt-1 block w-full border rounded px-3 py-2 text-sm">
        ...
        @foreach($gradeInputs as $input)
          <option value="{{ $input->id }}" {{ old('grade_input_id') == $input->id ? 'selected' : '' }}>
    ...
    <div class="mb-4">
      ...
      <textarea name="note" id="note" rows="3" class="mt-1 block w-full border rounded px-3 py-2 text-sm">{{ old('note') }}</textarea>
      @error('note')
        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
      @enderror
    </div>
    ...
    <div class="flex justify-end">
      ...
      <button type="submit" class="bg-blue-600 text-white text-sm px-4 py-2 rounded hover:bg-blue-700">Simpan</button>
    </div>
  </form>
</div>
@endsection
