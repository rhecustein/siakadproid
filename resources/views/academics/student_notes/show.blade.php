...
<div class="container mx-auto px-4 py-8 max-w-3xl">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Detail Catatan Siswa</h1>
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
            ...
            <div>
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <p class="font-medium">{{ $note->student->name }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">NIS</p>
                        <p class="font-medium">{{ $note->student->nis }}</p>
                    </div>
                </div>
            </div>
            
            <div>
                <h2 class="text-lg font-semibold text-gray-800 mb-4">Detail Catatan</h2>
                <div class="space-y-3">
                    <div>
                        <p class="text-sm text-gray-600">Jenis Catatan</p>
                        @php
                            $badgeColor = [
                    ...
                    <div>
                        ...
                        <p class="font-medium">{{ \Carbon\Carbon::parse($note->date)->format('d F Y') }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">Ditambahkan oleh</p>
                        <p class="font-medium">{{ $note->teacher->name ?? 'Admin' }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="mb-8">
            <h2 class="text-lg font-semibold text-gray-800 mb-2">Isi Catatan</h2>
        ...
        <div class="flex justify-end space-x-3">
            <a href="{{ route('academic.student-notes.edit', $note->id) }}" 
            ...
            <form action="{{ route('academic.student-notes.destroy', $note->id) }}" method="POST">
                @csrf
                @method('DELETE')
                <button type="submit" 
