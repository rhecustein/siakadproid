...
<div class="max-w-4xl mx-auto p-6">
    <div class="bg-white shadow-md rounded-xl p-6 space-y-6">
        ...
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-bold text-blue-700">✏️ Edit Jenjang Pendidikan</h2>
        ...
        <form method="POST" action="{{ route('academic.levels.update', $level->id) }}" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="slug" value="{{ old('slug', $level->slug) }}"
                ...
                <div>
                    ...
                    <input type="number" name="min_grade" value="{{ old('min_grade', $level->min_grade) }}"
                           class="mt-1 w-full border rounded-md p-2 @error('min_grade') border-red-500 @enderror">
                    @error('min_grade')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="max_grade" value="{{ old('max_grade', $level->max_grade) }}"
                           class="mt-1 w-full border rounded-md p-2 @error('max_grade') border-red-500 @enderror">
                    @error('max_grade')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                          ...
                          placeholder="Deskripsi tambahan...">{{ old('description', $level->description) }}</textarea>
                @error('description')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
